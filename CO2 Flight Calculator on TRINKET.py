...
print("Welcome to the CO2 Flight Emissions Calculator! This calculator is case-sensitive, so follow the prompts that will be shown.")
print("\n")
...
def airport_one():
    while True:
        list_1=[]
        search = input("Type 3-Digit Departure Airport Code (All Caps): ")
        f=open('airports.txt',encoding="utf-8")
...
def airport_two():
    while True:
        list_2=[]
        search = input("Type 3-Digit Destination Airport Code (All Caps): ")
        f=open('airports.txt',encoding="utf-8")
        for a, b, c, d, e, f, g, h, i, j, k, l, m, n in csv.reader(f):
            if e == search:
                print("Airport:", b,",",c,",",d,".")
...
def seat_class():
    class_list = []
    while True:
        question = input("Select class: Type 'E' for Economy, 'PE' for Premium Economy, 'B' for Business, or 'F' for First. ")
        if question == "E":
            proportion = 1.00
            while True:
...
def return_trip():
    while True:
        question = input("Select option for trip: Type '1' for 1-Way or '2' for 2-Way. ")
        if question == "2":
            trip = 2
            break
        elif question == "1":
...
#Converts to metric tons
    ...
    print("\n--------------------------------------------------------------------------------------------------")
    print("Total CO2 emissions produced by plane: {} metric tons".format(round(total_emission,2)))
    print("CO2 emissions produced by {} passenger(s) in {} class: {} metric tons".format(passenger_account, section_class, round(final_proportion_emission,2)))
...
#Passenger vehicle and smartphone data via https://www.epa.gov/energy/greenhouse-gas-equivalencies-calculator
    total_emission_tons = total_emission * 1.102
#^Convers to American tons
    total_emission_trees = total_emission_tons * 64
#^Tree equivalent derived from https://savingnature.com/offset-your-carbon-footprint-carbon-calculator/
    ...
    print("In addition, {} trees are needed to offset the {} metric tons of CO2 emissions, which is also equal to that of {} smartphones being fully charged.".format(round(total_emission_trees,0), round(final_proportion_emission, 2), smartphone_charge_format))
    print("\nNow, time to take action and offset your emissions. Take a look at some of the innovative ideas & projects that are helping tackle climate change below:")
    print("https://marketplace.goldstandard.org/collections/projects")
    print("https://www.planvivo.org/Pages/Category/projects?Take=27")
...
def aquit():
    while True:
        decision = input("\nPrint 'quit' to the console to quit. ")
        if decision == "quit":
            break
        else:
            continue
